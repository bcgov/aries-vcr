...
class Routing_Middleware_TestCase(TestCase):
    ...
    def test_routing_non_api_call(self):
        ...
        self.assertEqual(result[0].path_info, self.request.path_info)
    ...
    def test_routing_api_call_header_version(self):
        ...
        self.assertEqual(result[0].path_info, "/api/v2/some-incredible-method")
    ...
    def test_get_coalesced_request_version_noheader_nopath(self):
        ...
        self.assertEqual(
            result, settings.HTTP_HEADER_ROUTING_MIDDLEWARE_VERSION_MAP["default"]
        )
    ...
    def test_get_coalesced_request_version_header_path_matching(self):
        result = self.routing_middleware.get_coalesced_request_version(
            "latest", "latest"
        )
    ...
    def test_get_coalesced_request_version_header_path_conflicting(self):
        ...
        self.assertEquals(
            str(ex),
            "Conflicting API versions were requested in Accept header [latest] and path [default].",
        )

    def test_extract_header_version_noheader(self):
        header_version, request_meta_accept = (
            self.routing_middleware.extract_header_version(self.request)
        )
    ...
    def test_extract_header_version_header_toomany(self):
        ...
        ex = cm.exception
        self.assertEquals(
            str(ex), "Too many versions were specified in the Accept header."
        )
